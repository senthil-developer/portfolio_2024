...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn(
        "grid w-full grid-cols-1   py-10  md:grid-cols-2 lg:grid-cols-3",
        className,
      )}
    >
      {items.map((item, idx) => (
        <Button as={"div"} borderRadius="1.75rem">
          <Link
            href={item?.link}
            key={item?.link}
          ...
          >
            <AnimatePresence>
              {hoveredIndex === idx && (
                <motion.span
                  className="absolute inset-0 block h-full w-full rounded-3xl  bg-green-300/[0.7]"
